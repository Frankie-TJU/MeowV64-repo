...
object DecodeInfo {
  ...
  val table: Array[(BitPat, List[BitPat])] =
    Array(
      ...
      AMOSWAP_W -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOADD_W  -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOXOR_W  -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOAND_W  -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOOR_W   -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOMIN_W  -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOMAX_W  -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOMINU_W -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOMAXU_W -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),

      // RV64A Standard Extension (in addition to RV32A)
      LR_D      -> List(Y, N, Y, integer, Y, integer, N, XX, N, XX, lsu, mem),
      SC_D      -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOSWAP_D -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOADD_D  -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOXOR_D  -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOAND_D  -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOOR_D   -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOMIN_D  -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOMAX_D  -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOMINU_D -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      AMOMAXU_D -> List(Y, N, Y, integer, Y, integer, Y, integer, N, XX, lsu, mem),
      ...
      FNMADD_S  -> List(Y, N, Y, float, Y, float, Y, float, Y, float, fma, float),
      FNMSUB_S  -> List(Y, N, Y, float, Y, float, Y, float, Y, float, fma, float),
      ...
      FDIV_S    -> List(Y, N, Y, float, Y, float, Y, float, N, XX, floatDivSqrt, float),
      FSQRT_S   -> List(Y, N, Y, float, Y, float, N, XX, N, XX, floatDivSqrt, float),
      FSGNJ_S   -> List(Y, N, Y, float, Y, float, Y, float, N, XX, floatMisc, float),
      FSGNJN_S  -> List(Y, N, Y, float, Y, float, Y, float, N, XX, floatMisc, float),
      FSGNJX_S  -> List(Y, N, Y, float, Y, float, Y, float, N, XX, floatMisc, float),
      FMIN_S    -> List(Y, N, Y, float, Y, float, Y, float, N, XX, floatMisc, float),
      FMAX_S    -> List(Y, N, Y, float, Y, float, Y, float, N, XX, floatMisc, float),
      FCVT_W_S  -> List(Y, N, Y, integer, Y, float, N, XX, N, XX, floatMisc, float),
      FCVT_WU_S -> List(Y, N, Y, integer, Y, float, N, XX, N, XX, floatMisc, float),
      FMV_X_W   -> List(Y, N, Y, integer, Y, float, N, XX, N, XX, floatMisc, float),
      FEQ_S     -> List(Y, N, Y, integer, Y, float, Y, float, N, XX, floatMisc, float),
      FLT_S     -> List(Y, N, Y, integer, Y, float, Y, float, N, XX, floatMisc, float),
      FLE_S     -> List(Y, N, Y, integer, Y, float, Y, float, N, XX, floatMisc, float),
      FCLASS_S  -> List(Y, N, Y, integer, Y, float, N, XX, N, XX, floatMisc, float),
      FCVT_S_W  -> List(Y, N, Y, float, Y, integer, N, XX, N, XX, floatMisc, float),
      FCVT_S_WU -> List(Y, N, Y, float, Y, integer, N, XX, N, XX, floatMisc, float),
      FMV_W_X   -> List(Y, N, Y, float, Y, integer, N, XX, N, XX, floatMisc, float),

      // RV64F Standard Extension (in addition to RV32F)
      FCVT_L_S  -> List(Y, N, Y, integer, Y, float, N, XX, N, XX, floatMisc, float),
      FCVT_LU_S -> List(Y, N, Y, integer, Y, float, N, XX, N, XX, floatMisc, float),
      FCVT_S_L  -> List(Y, N, Y, float, Y, integer, N, XX, N, XX, floatMisc, float),
      FCVT_S_LU -> List(Y, N, Y, float, Y, integer, N, XX, N, XX, floatMisc, float),
      ...
      FNMADD_D  -> List(Y, N, Y, float, Y, float, Y, float, Y, float, fma, float),
      FNMSUB_D  -> List(Y, N, Y, float, Y, float, Y, float, Y, float, fma, float),
      ...
      FDIV_D    -> List(Y, N, Y, float, Y, float, Y, float, N, XX, floatDivSqrt, float),
      FSQRT_D   -> List(Y, N, Y, float, Y, float, N, XX, N, XX, floatDivSqrt, float),
      FSGNJ_D   -> List(Y, N, Y, float, Y, float, Y, float, N, XX, floatMisc, float),
      FSGNJN_D  -> List(Y, N, Y, float, Y, float, Y, float, N, XX, floatMisc, float),
      FSGNJX_D  -> List(Y, N, Y, float, Y, float, Y, float, N, XX, floatMisc, float),
      FMIN_D    -> List(Y, N, Y, float, Y, float, Y, float, N, XX, floatMisc, float),
      FMAX_D    -> List(Y, N, Y, float, Y, float, Y, float, N, XX, floatMisc, float),
      FCVT_S_D  -> List(Y, N, Y, float, Y, float, N, XX, N, XX, floatMisc, float),
      FCVT_D_S  -> List(Y, N, Y, float, Y, float, N, XX, N, XX, floatMisc, float),
      FEQ_D     -> List(Y, N, Y, integer, Y, float, Y, float, N, XX, floatMisc, float),
      FLT_D     -> List(Y, N, Y, integer, Y, float, Y, float, N, XX, floatMisc, float),
      FLE_D     -> List(Y, N, Y, integer, Y, float, Y, float, N, XX, floatMisc, float),
      FCLASS_D  -> List(Y, N, Y, integer, Y, float, N, XX, N, XX, floatMisc, float),
      FCVT_W_D  -> List(Y, N, Y, integer, Y, float, N, XX, N, XX, floatMisc, float),
      FCVT_WU_D -> List(Y, N, Y, integer, Y, float, N, XX, N, XX, floatMisc, float),
      FCVT_D_W  -> List(Y, N, Y, float, Y, integer, N, XX, N, XX, floatMisc, float),
      FCVT_D_WU -> List(Y, N, Y, float, Y, integer, N, XX, N, XX, floatMisc, float),

      // RV64D Standard Extension (in addition to RV32D)
      FCVT_L_D  -> List(Y, N, Y, integer, Y, float, N, XX, N, XX, floatMisc, float),
      FCVT_LU_D -> List(Y, N, Y, integer, Y, float, N, XX, N, XX, floatMisc, float),
      FMV_X_D   -> List(Y, N, Y, integer, Y, float, N, XX, N, XX, floatMisc, float),
      FCVT_D_L  -> List(Y, N, Y, float, Y, integer, N, XX, N, XX, floatMisc, float),
      FCVT_D_LU -> List(Y, N, Y, float, Y, integer, N, XX, N, XX, floatMisc, float),
      FMV_D_X   -> List(Y, N, Y, float, Y, integer, N, XX, N, XX, floatMisc, float),
      ...
      SFENCE_VMA -> List(Y, N, N, XX, Y, integer, Y, integer, N, XX, branch, int),
      ...
      VLUXEI64_V -> List(Y, N, Y, vector, Y, integer, Y, vector, N, XX, lsu, mem),
      VSE64_V    -> List(Y, Y, N, XX, Y, integer, N, XX, Y, vector, lsu, mem),

      // Vector Integer
      VADD_VV  -> List(Y, N, Y, vector, Y, vector, Y, vector, N, XX, vectorAlu, vec),
      VADD_VI  -> List(Y, N, Y, vector, Y, vector, N, XX, N, XX, vectorAlu, vec),
      VADD_VX  -> List(Y, N, Y, vector, Y, vector, Y, integer, N, XX, vectorAlu, vec),
      VFMV_F_S -> List(Y, N, Y, float, N, XX, Y, vector, N, XX, vectorMisc, vec),
      VMV_X_S  -> List(Y, N, Y, integer, N, XX, Y, vector, N, XX, vectorMisc, vec),
      VFMV_S_F -> List(Y, N, Y, vector, Y, float, N, XX, N, XX, vectorMisc, vec),
      VMV_S_X  -> List(Y, N, Y, vector, Y, integer, N, XX, N, XX, vectorMisc, vec),
      VMV_V_V  -> List(Y, N, Y, vector, Y, vector, N, XX, N, XX, vectorMisc, vec),
      VMV_V_I  -> List(Y, N, Y, vector, N, XX, N, XX, N, XX, vectorMisc, vec),
      VMV_V_X  -> List(Y, N, Y, vector, Y, integer, N, XX, N, XX, vectorMisc, vec),
