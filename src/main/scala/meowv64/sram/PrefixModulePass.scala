...
object PrefixModulesPass extends Transform with DependencyAPIMigration {
  ...
  def mapModuleName(prefix: String, orig: String): String = {
    prefix + '_' + orig
  }
  ...
  override protected def execute(state: CircuitState): CircuitState = {
    ...
    prefixes match {
      ...
      case Seq(prefix) =>
        ...
        state.copy(circuit = c.copy(main = mapModuleName(prefix, c.main)))
      case other =>
  ...
  private def onModule(m: ir.DefModule, prefix: String): ir.DefModule =
    m match {
      case e: ir.ExtModule => e.copy(name = mapModuleName(prefix, e.name))
      case mod: ir.Module =>
        val name = mapModuleName(prefix, mod.name)
  ...
  private def onStmt(s: ir.Statement, prefix: String): ir.Statement = s match {
    case i: ir.DefInstance => i.copy(module = mapModuleName(prefix, i.module))
