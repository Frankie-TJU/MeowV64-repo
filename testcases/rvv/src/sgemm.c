...
// https://github.com/riscv-non-isa/rvv-intrinsic-doc/blob/main/examples/rvv_sgemm.c
...
__attribute__((aligned(64))) float a_array[MAX_BLOCKSIZE] = {
...
__attribute__((aligned(64))) float b_array[MAX_BLOCKSIZE] = {
    ...
    for (size_t j = 0; j < NLEN; ++j)
      for (size_t k = 0; k < KLEN; ++k)
        golden_array[i * NLEN + j] +=
            a_array[i * KLEN + k] * b_array[j + k * NLEN];
}
